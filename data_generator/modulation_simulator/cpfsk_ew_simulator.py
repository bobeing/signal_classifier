...
# Title: CPFSK EW Single Burst Simulator
# Author: Hongrae Kim
...
import cpfsk_ew_simulator_epy_block_0 as epy_block_0  # embedded python block
import h5py




class cpfsk_ew_simulator(gr.top_block):

    def __init__(self):
        gr.top_block.__init__(self, "CPFSK EW Single Burst Simulator", catch_exceptions=True)
    ...
    def initialize(self, save_path, samp_rate, sat1_snr, baud_rate):
        ...
        self.epy_block_0 = epy_block_0.blk(filename=save_path, signaltype='comm', modulation='cpfsk', samprate=self.samp_rate, datarate=self.baud_rate, snr=self.sat1_snr, freqshift=self.sat1_doppler)
        ...
        self.analog_cpfsk_bc_0 = analog.cpfsk_bc(0.1, 1, (int(samp_rate/baud_rate)))


        ##################################################
        # Connections
        ##################################################
        self.connect((self.analog_cpfsk_bc_0, 0), (self.channels_channel_model_0_0, 0))
...
def main(top_block_cls=cpfsk_ew_simulator, options=None):
    tb = top_block_cls()
