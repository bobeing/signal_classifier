...
# Title: BPSK EW Single Burst Simulator
# Author: Hongrae Kim
...
import bpsk_ew_simulator_epy_block_0 as epy_block_0  # embedded python block
import h5py




class bpsk_ew_simulator(gr.top_block):

    def __init__(self):
        gr.top_block.__init__(self, "BPSK EW Single Burst Simulator", catch_exceptions=True)
        ...
        self.var_constellation = var_constellation = digital.constellation_bpsk().base()
    ...
    def initialize(self, save_path, samp_rate, sat1_snr, baud_rate):
        ...
        self.epy_block_0 = epy_block_0.blk(filename=save_path, signaltype='comm', modulation='bpsk', samprate=self.samp_rate, datarate=self.baud_rate, snr=self.sat1_snr, freqshift=self.sat1_doppler)
        ...
        self.channels_channel_model_0_0 = channels.channel_model(
            ...
            noise_seed=0,
            block_tags=False)
        # self.analog_random_source_x_0 = blocks.vector_source_b(list(map(int, numpy.random.randint(0, 255, 50))), False)
...
def main(top_block_cls=bpsk_ew_simulator, options=None):
    tb = top_block_cls()
